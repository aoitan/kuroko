...
1. ユーザー要求から `project / issue / phase / act / evd / block` を埋める。
2. ファイル名を規則に従って決める（Issue無ければ `misc`）。
3. `checkpoint/` 配下にファイルが無ければ作成する。
4. `# Timeline` が無ければ先頭に追加する。
5. 現在時刻（ローカル）でエントリを生成し、末尾に追記する。
6. 追記後、ユーザーに以下を簡潔に報告する:
   - 追記したファイルパス
   - 追記した1エントリ（そのまま）
